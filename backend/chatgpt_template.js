...
function formatBirthChartForChatGPT(birthChart) {
  ...
  const planets =
    birthChart.planets && typeof birthChart.planets === "object"
      ? birthChart.planets
      : {};
  ...
  const aspectList = aspects
    .map(
      (aspect) =>
        `${planetNames[aspect?.planet1] || aspect?.planet1} (${aspect?.planet1Sign || "?"}) ${aspect?.aspect || "?"} ${planetNames[aspect?.planet2] || aspect?.planet2} (${aspect?.planet2Sign || "?"}) - ${aspect?.orb != null ? Number(aspect.orb).toFixed(1) : "?"}° orb`,
    )
    .join("\n");
